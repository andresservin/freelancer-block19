...
function render() {
  ...
  const circlesElements = circles.map((shape) => {
    const elements = document.createElement("li");
    elements.classList.add(shape.color, shape.size);
    return element;
  });
  circles.replaceChildren(...squareElements);
}
...
function addShape() {
  ...
  function addShape(){
    const size= sizes [Math.floor(Math.random() * sizeslength)];
  ...
  if (shapes.length === maxShapes){
    clearInterval(addShapeIntervslId);
  }
  clearInterval
}
